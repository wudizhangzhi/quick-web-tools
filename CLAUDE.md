...
- **Video Delogo / Crop Command** (`/tools/video-delogo`): 选本地视频 → 在画面上拖框选区域，支持两种模式：delogo（去台标/水印，可多框、可为每个框独立设置生效时间段）或 crop（裁切保留选定区域，单框）。生成对应 ffmpeg 命令拿去本地跑（使用 delogo 的 `enable='between(t,...)` / `gte(t,...)` timeline 机制）。纯客户端，视频不上传。纯逻辑在 `lib/video-delogo/command.ts`（Region 可带可选 startSec/endSec、buildEnableExpr、两种 clamp、delogo 链式或 crop=w:h:x:y、文件名加引号、按模式用 `_delogo` / `_crop` 后缀），有 vitest 覆盖。不做真实视频处理。crop 模式不暴露时间控制（避免中途分辨率变化）。
...
- 二次：`hupu_share` / `sub_share` / `invisible_unicode_share` / `force_yes_post_win` / `wc_share`（`{ status: 'save_image' | 'copy_image' | 'copy_link' }`）/ `video_delogo_share`（`{ status: 'copy_command', regions, mode, timedRegions }`） / `video_delogo_time`（`{ action: 'set_start' | 'set_end' | 'clear' }`）
