...
- **Video Delogo / Crop Command** (`/tools/video-delogo`): 选本地视频 → 在画面上拖框选区域，支持两种模式：delogo（去台标/水印，可多框）或 crop（裁切保留选定区域，单框）。生成对应 ffmpeg 命令拿去本地跑。纯客户端，视频不上传。纯逻辑在 `lib/video-delogo/command.ts`（两种 clamp、delogo 链式或 crop=w:h:x:y、文件名加引号、按模式用 `_delogo` / `_crop` 后缀），有 vitest 覆盖。不做真实视频处理 / 不做时间段裁剪。
